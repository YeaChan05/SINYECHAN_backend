...
- 도메인 모듈은 `model`, `infrastructure`, `service`, `exception`, `api`, `repository-{type}`,
  `application-{type}`로 구성합니다.
...
- `model`: Aggregate Root는 `Domain`, 구성 요소는 `DomainIdentity`/`DomainProps`, 값 객체는 `XXXValue`/
  `XXXAmount`/`XXXPolicy`.
...
- `api-internal`: `DomainInternalApiController`, `DomainInternalApiRequest`,
...
- `repository-{type}`: 구현체 `DomainRepositoryImpl`, 기술 리포지토리 `DomainJpaRepository`, 영속 엔티티
  `DomainJpaEntity`.
